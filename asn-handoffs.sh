...
echo "This script will output asn_handoffs_$TABLE.csv"
echo "tr_id_from | hop_from | ip_addr_from | hostname_from | asn_from | mm_lat_from | mm_lng_from | lat_from | lng_from | mm_city_from | mm_region_from | mm_country_from | mm_postal_from | gl_override_from | short_name_from | name_from | dest_from | dest_ip_from | sub_time_from | submitter_from | zip_code_from | hop_lh_from | ip_addr_lh_from | latency_from | tr_id_to | hop_to | ip_addr_to | hostname_to | asn_to | mm_lat_to | mm_lng_to | lat_to | lng_to | mm_city_to | mm_region_to | mm_country_to | mm_postal_to | gl_override_to | short_name_to | name_to | dest_to | dest_ip_to | sub_time_to | submitter_to | zip_code_to | hop_lh_to | ip_addr_lh_to | latency_to" >> asn_handoffs_$TABLE.csv


psql -d ixmaps -tc "select distinct traceroute_id from $TABLE" ixmaps |
while read -a DCANDATA ; do
    #echo "${DCANDATA[0]} is the set of trs in the table selected above"
    for trId in "${DCANDATA[0]}"
    do
        # use me for resuming the script part way if interrupted
        #if (($trId > 52646))
        #then
            echo Searching traceroute $trId"..."
            psql ixmaps -c "select traceroute_id,hop,asnum into table temp$trId from $TABLE where traceroute_id=$trId order by hop;"
            ...
            do
                ...
                then
                    ...
                    firstHopString="$(psql ixmaps -Atc "select * from $TABLE where traceroute_id=$trId and hop=$hopNum order by traceroute_id,hop;")"
                    ...
                    secondHopString="$(psql ixmaps -Atc "select * from $TABLE where traceroute_id=$trId and hop=$((hopNum+incrCounter)) order by traceroute_id,hop;")"
                    ...
                    echo $firstHopString"|"$firstHopMinLatency"|"$secondHopString"|"$secondHopMinLatency >> asn_handoffs_$TABLE.csv
                fi
            done
